...
export default function About({ onSave }) {
  ...
  const userData = JSON.parse(localStorage.getItem("walletUser"));
  ...
  const handleSubmit = (e) => {
    ...
    completeGmailUser(controller.user.id, data)
      .then(async (user) => {
      ...
      <SoftBox pt={2} mb={3} px={2}>
        <SoftBox display="flex" flexDirection="column" justifyContent="start">
          ...
          <SoftBox mb={1} lineHeight={0} textAlign="left">
            <SoftTypography variant="h6" fontWeight="light" color="text" textAlign="left">
