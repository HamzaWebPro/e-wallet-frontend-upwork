...
function Socials2() {
  ...
  function singGmail () {
    ...
    SingInGoogle2().then(async (data) => {
      ...
      // if(data.user.stripeAccount){
      //     await setUser(dispatch, data)
      //     await setCurrencys(dispatch, data)
      //     navegar("/dashboard")
      // }if(!data.user.stripeAccount && data.user.trparty){
      //     await setUser(dispatch, data)
      //     navegar("/activation-process-full")
...
{/*     <SoftButton variant="outlined" color="light" onClick={singFace}>
      ...
      <SoftButton variant="outlined" color="light" onClick={singGmail}>
        <svg width="24px" height="32px" viewBox="0 0 64 64" version="1.1">
          <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
